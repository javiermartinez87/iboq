...
function analiza($datos, &$p_limpia) {
    ...
    $sigo = reglas_clasificacion($keys, $owa_capitulos);
    if ($sigo != false) {
        $owa_subcapitulos = owa_subcapitulo($palabras, $sigo, $posicion_palabra);
...
function reglas_clasificacion($keys, $values, $trunca = 0) {
    if ($trunca == 0) {
        $dif = $values[$keys[0]] - $values[$keys[1]];
        $r1 = 'C' . $keys[0];
        $r2 = 'C' . $keys[1];
        $r3 = 'C' . $keys[2];
    }else{
    ...
    switch ($r1) {
        ...
        case 'C8':
            if ($dif <= 0.0526 && $r2 == 'C1') {
                return false;
            } elseif ($dif <= 0.0013 && $r2 == 'C3') {
                return false;
            } elseif ($dif <= 0.0185 && $r2 == 'C4') {
                return false;
            } elseif ($dif <= 0.061 && $r2 == 'C6') {
                return false;
            } elseif ($dif <= 0.006 && $r2 == 'C10') {
                return false;
            } elseif ($dif <= 0.024 && $r2 == 'C12') {
                return false;
            } elseif ($dif <= 0.0312 && $r2 == 'C15') {
                return false;
            }
            break;
        case 'C9':
            if ($dif <= 0.0156 && $r2 == 'C8' && $r3 == 'C10') {
                return false;
            }
            if ($dif <= 0.021 && $r2 == 'C8' && $r3 != 'C10') {
                return false;
            }
            break;
        case 'C10':
            if ($dif <= 0.034 && $r2 != 'C14' && $r2 == 'C12') {
                return false;
            }
            if ($dif <= 0.021 && $r2 == 'C8' && $r3 != 'C10') {
                return false;
        ...
        case 'C11':
            ...
            if ($dif <= 0.002 && $r2 != 'C8' && $r2 != 'C10') {
                return false;
            }
            break;
        case 'C12':
            if ($dif <= 0.0452 && $dif > 0.0356 && $r2 == 'C8') {
                return false;
            }
            break;
        case 'C13':
            if ($dif <= 0.045 && $r2 != 'C8' && $r2 != 'C9') {
                return false;
            }
            break;
        case 'C14':
...
function owa_capitulo($palabras, $posicion_palabra) {
    ...
    $las_palabras = (join("','", $palabras));
    $sql = "select distinct id from capitulo order by id ASC";
    $result = $mysqli->query($sql);
